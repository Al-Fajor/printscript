...
public class UnfinishedSeparatorsDetector implements LexicalErrorDetector {
    ...
    public ScanResult detect(String input) {
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            if (charAtI == '\"') {
                ...
                if (stack.isEmpty() || !matches(stack.peek(), charAtI)) {
                    return new FailedScanResult(i, "Unmatched closing character '" + charAtI + "' at index " + i);
                }
                stack.pop();
            }
        }
        ...
        if (!stack.isEmpty()) {
            return new FailedScanResult(input.length(),"Unmatched opening character '" + stack.peek() + "' at index " + input.lastIndexOf(stack.peek()));
        }
