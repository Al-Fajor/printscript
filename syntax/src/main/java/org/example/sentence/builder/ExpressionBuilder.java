    ...
    return switch (current.getType()){
      ...
      case SEPARATOR -> {
        if(mapper.matchesSeparatorType(current, "opening")){
          ...
          if(!mapper.matchesSeparatorType(reader.getCurrentToken(), "closing")) yield null;
          reader.consume();
          yield expression;
        }
        yield null;
      ...
      case OPERATOR -> {
        if(current.getValue().equals("-")){
          ...
          reader.consume();
          yield mapper.mapToken(new Token(newCurrent.getType(), newCurrent.getStart(), newCurrent.getEnd(), "-" + newCurrent.getValue()));
        }
        yield parseExpression(reader, getBindingPower(current));
      }
      default -> throw new IllegalStateException("Cannot build primary expr from type: " + current.getType());
    };
  }

  private EvaluableComponent parseBinaryExpression(TokenReader tokenReader,EvaluableComponent left, int bindingPower){
    Token currentToken = tokenReader.getCurrentToken();
//    if(operator.getType() != OPERATOR || operator == null || getBindingPower(operator) < bindingPower) return left;
    tokenReader.consume();
    EvaluableComponent right = parseExpression(tokenReader, bindingPower);


    TokenMapper mapper = new TokenMapper();
    return left != null ? new BinaryExpression(mapper.mapOperator(currentToken.getValue()), left, right) :
