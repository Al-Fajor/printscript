...
public class StatementVisitor implements org.example.ast.visitor.StatementVisitor<Void> {
	...
	public Void visit(DeclarationStatement statement) {
		...
		switch (declarationType) {
			case STRING -> state.addStringVariable(new Variable<>(declarationType, identifier.getName(), null));
			case NUMBER -> state.addNumericVariable(new Variable<>(declarationType, identifier.getName(), null));
			default -> throw new RuntimeException("Unexpected declaration type: " + declarationType);
		}
		return null;
	}
	...
	public Void visit(DeclarationAssignmentStatement statement) {
		...
		DeclarationStatement declarationStatement = new DeclarationStatement(declarationType, identifierType, identifier, statement.getStart(), statement.getEnd());
		this.visit(declarationStatement);

		EvaluableComponent evaluableComponent = statement.getEvaluableComponent();
		AssignmentStatement assignmentStatement = new AssignmentStatement(identifier, evaluableComponent, statement.getStart(), statement.getEnd());
