...
public class AssigningToValidValue extends ConditionalValidator {
	...
	protected boolean meetsCondition(Environment environment) {
		return leftResolution
				.type()
				.or(() -> Optional.of(environment.getDeclarationType(leftResolution.name())))
				.flatMap(value1 -> rightResolution.evaluatedType().map(value2 -> value1 == value2))
				.orElseThrow(() -> new IllegalStateException("Cannot perform type comparison"));
	}
}
